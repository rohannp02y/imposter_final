...
import { GamepadIcon, HomeIcon } from "@/components/icons/SvgIcons";

export default function PlayPage() {
  const router = useRouter();

  useEffect(() => {
    const gameData = localStorage.getItem("imposter_passplay_game");
    if (gameData) router.push("/game/pass-and-play/play");
  }, [router]);
    ...
    <div className="min-h-screen pt-20 pb-8 px-6 flex items-center justify-center relative">
      ...
      >
        <div className="card-surface p-8 text-center">
          ...
          <h1 className="text-display text-2xl text-ink-primary mb-2">No Active Session</h1>
          <p className="text-ink-muted text-sm font-mono mb-8">Start a new game to play</p>

          <div className="space-y-3">
            <Link href="/game/pass-and-play/setup" className="btn-primary w-full flex items-center justify-center gap-2">
              <GamepadIcon size={16} /> New Game
            </Link>
